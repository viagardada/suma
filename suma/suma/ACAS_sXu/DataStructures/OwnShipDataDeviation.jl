mutable struct OwnShipDataDeviation
...
OwnShipDataDeviation() =
new(0, false, NaN, zeros(2), zeros(2,2), NaN, false, OWN_HEADING_INVALID, 0, zeros(2), zeros(2,2),
0, 0, NaN, NaN, NaN, NaN, NaN, NaN, NaN, zeros(4,4), NaN, OWN_WGS84_INVALID, zeros(2),
zeros(2,2), NaN, 0, 0, NaN, NaN, TRM_VELMODE_NONE, NaN, OPMODE_STANDBY, true, 0,
fill(false,4), fill(false,2), CorrelationHistory(Z), CorrelationHistory(Z),
CorrelationHistory(String), CorrelationHistory(String), TrackSummary(),
GeodeticStates(), GeodeticStates(), AGTTrackFile[], OwnHistory(),
OwnDiscreteData(), V2VOperationalStatusMessage(), [-Inf], [-Inf]
)
end
