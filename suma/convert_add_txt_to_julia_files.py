...
###############################################################
# Taken from UMA (ACAS Xu).
#
# Purpose of this script is to exctract all functions and
...
#
# Some manual steps are needed afterwards, including:
# * Removing extra newlines that would change the code meaning
# * Updating any types (e.g. from typeof) that were converted to string
#   (removing the ACAS_Xu namespace)
##
...
def LineTransformParamsCall(line):
    ...
    if pattern_:
        ...
        for param_name in param_groups:
            ...
            if mode_ptrn:
                replacement += '["' + \
                    param_name.replace(mode_ptrn.group(), '') + '"]'
...
def AddThisToFncs(db):
    ...
    for i in db:
        if db[i].thisParam and not db[i].useThis:
            ...
            print(('thisParam: {}, useThis: {}'.format(
                db[i].thisParam, db[i].useThis)))
            raise RuntimeError

        for y in range(len(db[i].content)):
            if db[i].useThis and db[i].thisParam:
                if db[i].name + '()' in db[i].content[y]:
                    db[i].content[y] = db[i].content[y].replace(
                        db[i].name + '(', db[i].name + '(this::' + db[i].thisParam)
...
def GenFiles(db):
    ...
    for i in db:
        with open(os.path.join(db[i].dirPath, db[i].fileName), 'w') as newFile:
...
if __name__ == '__main__':
    ...
    with open(parameters.FileName[0]) as codeFile:
        ...
        for idx in range(len(originalCode) - 1):
            ...
            if RecordAllAlgorithms:
                m = match(r'\d*\s*\d+\s+(\w+)\s+\d+', line)
                if m:
                    pureAlgos.append(m.groups()[0])
                    continue

            l = next((c for c in chapters if c.rstrip() in line), None)
            # set designated directory
            if l:
                dirPath = os.path.join(cwd, root_f, chapters.get(l))
            ...
            if (match(r'\s*1\s+function', line) or match(r'\s*1\s+type',
                                                         line) or match(r'\s*1\s+abstract', line)):
                newFnc = True
            ...
            if not newFnc and lineVec:
                ...
                ft_db[newFileName[:-3]
                      ] = ffile(newFileName, fldr, dirPath, usedFnc, lineVec)
